  ...
  if (variant ==="card") {
    return (
      <section className="border border-[#e5e5e5] bg-white p-6 md:p-8">
        <p className="text-[0.625rem] font-mono text-[#0a0a0a] uppercase tracking-[0.22em] mb-2">
          Newsletter
        </p>
        <h3 className="text-xl md:text-2xl font-bold text-[#0a0a0a] mb-2">
          {headline ||"5-minute teardowns, every Tuesday."}
        </h3>
        <p className="text-sm text-[#737373] mb-5 max-w-md">
          {description ||
"One SaaS landing page broken down per email. Three concrete fixes you can apply to your own page that day. Unsubscribe anytime."}
  ...
  return (
    <section className="border border-[#e5e5e5] bg-[#fafafa]/40 p-5 my-10">
      <p className="text-[0.625rem] font-mono text-[#0a0a0a] uppercase tracking-[0.22em] mb-2">
        Get the next one
      </p>
  ...
  return (
    <form onSubmit={submit} className="flex flex-col sm:flex-row gap-2">
      <input
        ...
        autoComplete="email"
        className={`flex-1 px-3 ${compact ?"py-2" :"py-3"} text-sm border border-[#e5e5e5] bg-white focus:outline-none focus:border-[#0a0a0a] focus:ring-1 focus:ring-[#0a0a0a]`}
      />
      <button
        type="submit"
        disabled={status ==="sending"}
        className={`inline-flex items-center justify-center gap-1.5 px-4 ${compact ?"py-2" :"py-3"} bg-[#0a0a0a] text-white text-xs font-bold hover:bg-[#0a0a0a] transition-colors disabled:opacity-60`}
      >
        {status ==="sending" ? (
